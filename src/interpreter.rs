...
impl Interpreter {
    pub fn current_namespace(&self) -> &Identifier {
        self.namespaces.current_namespace_name()
    }
    ...
    pub fn load_namespace(
        ...
        self.namespaces.intern_namespace(&name, namespace);
    ...
    fn intern(&mut self, symbol: &Symbol, value: Option<RuntimeValue>) -> EvaluationResult<Var> {
        let namespace = match &symbol.namespace {
            Some(ns) => self.namespaces.get_namespace_mut(ns),
            None => self.namespaces.current_namespace_mut(),
        };
    ...
    fn resolve_symbol_to_value(&self, symbol: &Symbol) -> EvaluationResult<RuntimeValue> {
        // TODO: fixme
        // let var = self.namespaces.resolve_symbol(symbol)?;
        // match var {
        //     Var::Bound(value) => value.clone(),
        //     Var::Unbound => panic!("hmmm"),
        // }
        Ok(RuntimeValue::Nil)
    }
    ...
    // fn eval_try_legacy(&mut self, operand_forms: PersistentList<Value>) -> EvaluationResult<Value> {
    ...
    // fn eval_list_legacy(&mut self, forms: &PersistentList<Value>) -> EvaluationResult<Value> {
    //     if forms.is_empty() {
    ...
    fn extend_lexical_scope(&mut self, bindings: impl Iterator<Item = (Identifier, RuntimeValue)>) {
        let scope = Scope::from_iter(bindings);
    ...
    fn eval_let(&mut self, let_form: &LetForm) -> EvaluationResult<RuntimeValue> {
        let lexical_form = &let_form.lexical_form;
        ...
        if has_forward_declarations {
            self.extend_lexical_scope(forward_declarations.iter().map(|&index| {
                (
                    let_form.identifier_for_binding(index).unwrap().clone(),
        ...
        let bindings = match &lexical_form.bindings {
            LexicalBindings::Bound(bindings) => bindings
                .iter()
                .map(|(identifier, form)| {
                    Ok((identifier.clone(), self.evaluate_analyzed_form(form)?))
                })
                .collect::<Result<Vec<(Identifier, RuntimeValue)>, EvaluationError>>()
        ...
        self.extend_lexical_scope(bindings.into_iter());
    ...
    fn eval_loop(&mut self, lexical_form: &LexicalForm) -> EvaluationResult<RuntimeValue> {
        let bindings = match &lexical_form.bindings {
            LexicalBindings::Bound(bindings) => bindings
                .iter()
                .map(|(identifier, form)| {
        ...
        while let ControlFlow::Recur(values) = self.control_stack.pop().unwrap() {
            ...
            let bindings = lexical_form.bindings.names();
            for (name, value) in bindings.iter().zip(values.into_iter()) {
    ...
    fn eval_catch(
        ...
        let bindings = [(exception_binding.clone(), exception)];
        self.extend_lexical_scope(bindings.into_iter());
    ...
    fn apply_fn(
        ...
        let variable = args.drain(f.arity()..).collect::<Vec<_>>();

        let bindings = f.parameters.iter().cloned().zip(args.into_iter());
        if f.variadic() {
            let variadic_binding = f.variadic.as_ref().unwrap().clone();
            let variadic_arg = RuntimeValue::List(PersistentList::from_iter(variable));

            self.extend_lexical_scope(bindings.chain([(variadic_binding, variadic_arg)]));
        } else {
            self.extend_lexical_scope(bindings);
        }
    ...
    fn evaluate_analyzed_form(&mut self, form: &RuntimeValue) -> EvaluationResult<RuntimeValue> {
        let value = match form {
            ...
            RuntimeValue::Var(var) => match var {
                Var::Bound(value) => value.borrow().clone(),
                Var::Unbound => RuntimeValue::Var(Var::Unbound),
            },
    ...
    pub(crate) fn analyze_and_evaluate(&mut self, form: &Form) -> EvaluationResult<RuntimeValue> {
        let mut analyzer = Analyzer::new(&self.namespaces);
